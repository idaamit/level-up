...
import org.sparkaton.levelup.dto.UserStatistics;
...
public class UserService {
    ...
    public UserStatistics getUserStatistics(int userId) {
        Map<Integer, UserQuiz> userQuizzes = DB.getUserQuiz(userId);
        int rightAnswers = 0, wrongAnswers = 0;
        for (UserQuiz userQuiz : userQuizzes.values()) {
            if (userQuiz.getUserAnswers() != null) {
                for (UserAnswer userAnswer : userQuiz.getUserAnswers()) {
                    if (userAnswer.isWasAnsweredCorrectly())
                        rightAnswers++;
                    else
                        wrongAnswers++;
                }
        ...
        int score=1000*(userQuizzes.values().size()+rightAnswers)/(rightAnswers+wrongAnswers);
        UserStatistics userStatistics = new UserStatistics(userId, score,userQuizzes.values().size(),rightAnswers,wrongAnswers);
        return userStatistics;
    }

    private void setDefaultUserQuizzes() {
